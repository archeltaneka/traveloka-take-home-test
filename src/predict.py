...
arg_parser.add_argument('--user_lat', required=True, help='Current latitude location')
arg_parser.add_argument('--user_lon', required=True, help='Current longitude location')
...
try:
    recommended_vendors = recommend_a_user(model, args['user_id'], float(args['user_lat']), float(args['user_lon']), vendors, user_profile, vendor_profile)
